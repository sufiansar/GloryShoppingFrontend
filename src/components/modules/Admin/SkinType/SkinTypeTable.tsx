...
import { MoreVertical, Search, Trash2, Edit3, Calendar, FileText, Fingerprint, ShieldCheck, RefreshCw } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl rounded-3xl border border-white/40 dark:border-slate-800/50 shadow-sm overflow-hidden">
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b border-slate-200/30 dark:border-slate-800/30 hover:bg-transparent">
                <TableHead className="py-4 pl-8 text-xs font-semibold text-slate-500 uppercase tracking-wider">Profile Name</TableHead>
                <TableHead className="py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Data Description</TableHead>
                <TableHead className="py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Registration Date</TableHead>
                <TableHead className="text-right py-4 pr-8 text-xs font-semibold text-slate-500 uppercase tracking-wider">Operations</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {loading ? (
                <TableRow>
                  <TableCell colSpan={4} className="text-center py-24">
                     <div className="flex flex-col items-center gap-4">
                        ...
                        <span className="text-sm font-medium text-slate-400">Loading bio-data...</span>
                     </div>
                  </TableCell>
                </TableRow>
              ) : skinTypes.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={4} className="text-center py-24">
                     <div className="flex flex-col items-center gap-2">
                        <Fingerprint className="h-10 w-10 text-slate-200" />
                        <span className="text-sm font-semibold text-slate-600">No Profiles Found</span>
                ...
                skinTypes.map((skinType) => (
                  <TableRow key={skinType.id} className="premium-table-row border-b border-slate-100/30 dark:border-slate-800/20 group/row">
                    <TableCell className="py-4 pl-8">
                       <div className="flex items-center gap-4">
                          <div className="h-10 w-10 rounded-xl bg-primary-custom/10 flex items-center justify-center border border-primary-custom/20 group-hover/row:scale-105 transition-all duration-300">
                             <Fingerprint className="h-5 w-5 text-primary-custom" />
                          </div>
                          <div className="flex flex-col">
                            <span className="text-sm font-bold text-slate-900 dark:text-white leading-tight">{skinType.name}</span>
                            <span className="text-xs text-slate-400">ID: {skinType.id.slice(0, 8)}</span>
                          </div>
                       </div>
                    </TableCell>
                    <TableCell>
                       <div className="flex items-center gap-3 max-w-[400px]">
                          ...
                          <p className="text-sm text-slate-500 line-clamp-2 leading-relaxed">
                            {skinType.description || "No description provided."}
                          </p>
                       </div>
                    </TableCell>
                    <TableCell>
                       <div className="flex items-center gap-2 text-slate-500">
                          ...
                          <span className="text-sm font-medium">
                            {new Date(skinType.createdAt).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}
                          </span>
                       </div>
                    </TableCell>
                    <TableCell className="text-right pr-10">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-56 p-2 rounded-2xl border-white/20 dark:border-slate-800/50 backdrop-blur-3xl glass-card">
                          <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-3 py-2">Classification Tools</DropdownMenuLabel>
        ...
        <Pagination
          ...
          totalItems={skinTypes.length > 0 ? (totalPages > 1 ? totalPages * 10 : skinTypes.length) : 0}
          itemsPerPage={10}
          onPageChange={fetchSkinTypes}
        ...
        <AlertDialogContent className="max-w-md p-0 overflow-hidden premium-glass-dialog border-none rounded-[3rem]">
          <div className="p-10 flex flex-col items-center text-center gap-8">
             ...
             <div className="space-y-2">
                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-rose-500">System Alert: Purge Sequence</span>
                <AlertDialogTitle className="text-3xl font-black uppercase tracking-tight text-slate-900 dark:text-white leading-tight">Confirm Purge</AlertDialogTitle>
                <AlertDialogDescription className="font-medium text-slate-500 text-sm max-w-[300px] mx-auto leading-relaxed">
                  Are you absolutely sure you want to purge this bio-classification? This action is non-reversible and will permanently erase the data.
                </AlertDialogDescription>
             </div>
            ...
            <div className="flex flex-col w-full gap-3">
              <Button 
                ...
                className="h-16 rounded-2xl bg-rose-500 text-white font-black uppercase tracking-[0.2em] text-[10px] hover:bg-rose-600 transition-all shadow-xl shadow-rose-500/20 active:scale-[0.98]"
              >
                Execute Purge
              </Button>
              <AlertDialogCancel className="h-14 rounded-2xl border-slate-200 dark:border-slate-800 font-black uppercase tracking-[0.2em] text-[10px] hover:bg-slate-100 dark:hover:bg-slate-800 transition-all active:scale-[0.98] bg-transparent">
                Abort Command
              </AlertDialogCancel>
            </div>
          </div>
